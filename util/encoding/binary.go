...
func Read(r *bytes.Buffer, order ByteOrder, data interface{}) (actualRead int,err error) {
	// Fast path for basic types and slices.
	if n := IntDataSize(data); n != 0 {
		bs,err := r.NeedDataForRead(n)
		if err!=nil {
			return n,err
		}
	...
	if size < 0 {
		return size,errors.New("binary.Read: invalid type " + reflect.TypeOf(data).String())
	}

	buf,err:=r.NeedDataForRead(size)
	if err!=nil {
		return size,err
	}
